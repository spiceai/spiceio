...
- Multi-instance safety — startup cleanup only sweeps WAL temps / upload dirs older than `SPICEIO_CLEANUP_GRACE_SECS`, judged on the *server's* clock (measured by writing a probe file, so clock skew can't age a live file out) — several instances can serve one share. The reaper refreshes each live upload's marker file so a slow client's upload never looks abandoned. Read/stat/copy opens retry a missing leaf name once to absorb a peer process's brief WAL rename/close interval; a genuine missing leaf therefore costs one extra open, with no fixed sleep. Missing parents and invalid paths return immediately. Local publication locks coordinate only clones of the same session; the bounded retry does not provide distributed locking or cover an indefinitely stalled publication.
