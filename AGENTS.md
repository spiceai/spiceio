...
- Existence index is the miss-path lever for sccache — a cold compile unit GETs a unique hash once, so a negative 404 cache cannot help. A complete name set per directory, filled by one QUERY_DIRECTORY on the first miss, makes later missing leaves 404 with no SMB open (and no publication retry). PUT/DELETE maintain an already-listed set; an unlisted directory is left incomplete so one PUT cannot mark the rest empty. Same contract as immutable objects: a peer's unpublished PUT 404s until the next list. Default on with immutable; `SPICEIO_EXISTENCE_INDEX` overrides.
...
- Multi-instance safety — startup cleanup only sweeps WAL temps / upload dirs older than `SPICEIO_CLEANUP_GRACE_SECS`, judged on the *server's* clock (measured by writing a probe file, so clock skew can't age a live file out) — several instances can serve one share. The reaper refreshes each live upload's marker file so a slow client's upload never looks abandoned. Read/stat/copy opens retry a missing leaf name twice to absorb a peer process's WAL rename/close interval: immediately, then after 4ms if the name is still gone. LAN RTT can be shorter than that interval, so the wait is required — a single extra open with no sleep 404'd live range-GETs while a peer replaced a 128 KiB key. A genuine missing leaf therefore costs two extra opens and 4ms. Missing parents and invalid paths return immediately. Local publication locks coordinate only clones of the same session; the bounded retry does not provide distributed locking or cover an indefinitely stalled publication.
